...
**Lighthouse HTML / log-analysis:** `organize-reports.sh` gom ở root → `lighthouse-reports/`: `lighthouse-report*.html` và `*.report.html` (vd. `your-app.vercel.app_….report.html`). `log-analysis-*.json` → `reports/log-analysis/`. `run-lighthouse.sh` và `log-analyzer.js` ghi thẳng đúng chỗ mới.
